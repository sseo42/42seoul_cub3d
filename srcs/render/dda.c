...
void		dda_init(t_canvas *canvas_ptr, double ray_vec[2], int loc_int[4], double distance[4])
{
...
int			dda_loop(t_canvas *canvas_ptr, t_obj_info **obj_info, int loc_int[4], double distance[4])
{
	int			side;
	int			label;
	t_obj		*obj;
	...
	while (1)
	{
		...
		else if (label > 1)
		{
			...
			if ((info_idx = search_obj_info(*obj_info, obj->id)))
				...
				if (put_obj_info(obj_info, obj, canvas_ptr->x_loc, canvas_ptr->y_loc))
					return (2);
		}
...
void		dda(t_canvas *canvas_ptr, t_wall_info *wall_info, t_obj_info **obj_info, double ray_vec[2])
{
	int			side;
	int			direction;
	int			loc_int[4]; //x, y, dx, dy
	double		distance[4]; //dis_x, dis_y, dis_dx, dis_dy
	double		hit_point;
	...
	if (side > 1)
		exit (0);
	direction = ray_vec[side] < 0 ? side * 2 + 1 : side * 2;
	wall_info->wall_ptr = search_img(canvas_ptr->imgs, (canvas_ptr->map)[loc_int[1]][loc_int[0]], direction);
	wall_info->distance = distance[side];
	wall_info->line_height = (int)(canvas_ptr->height / distance[side]);
	wall_info->draw_start = -wall_info->line_height / 2 + canvas_ptr->horizon_line;
	...
	if (side)
		...
		(canvas_ptr->y_loc - loc_int[1] + direction - 3) * ray_vec[0] / ray_vec[1];
	else
		hit_point = (ray_vec[0] == 0) ? canvas_ptr->y_loc : canvas_ptr->y_loc + \
		(canvas_ptr->x_loc - loc_int[0] - direction) * ray_vec[1] / ray_vec[0];
	wall_info->wall_x = (int)((hit_point - floor(hit_point)) * wall_info->wall_ptr->width);
}
