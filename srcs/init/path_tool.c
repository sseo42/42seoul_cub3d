...
/*   Updated: 2020/07/23 11:21:15 by sseo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				get_resolution(t_canvas *canvas_ptr, const char *line)
{
	...
	if (!(line = check_resolution_line(line, &width, screen_width)))
		return (print_error("Wrong resolution"));
	if (!(line = check_resolution_line(line, &height, screen_height)))
		return (print_error("Wrong resolution"));
	while (*line == ' ')
		line++;
...
int				get_wall_texture(t_canvas *canvas_ptr, const char *line, int d)
{
	...
	if (!(texture_path = trim_space(line)))
		return (1);
	if (!get_rgb(canvas_ptr, texture_path, d))
		return (0);
